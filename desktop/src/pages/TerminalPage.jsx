...
export default function TerminalPage({ client, config }) {
  ...
  useEffect(() => {
    ...
    term.writeln('\x1b[90m  然后创建 ACL 文件（注意 key 必须是 root）：\x1b[0m')
    term.writeln('\x1b[90m  cat > /usr/share/rpcd/acl.d/owm.json << EOF\x1b[0m')
    term.writeln('\x1b[90m  {"root":{"read":{"ubus":{"*":["*"]},"uci":{"*":["read"]},"file":{"/tmp/dhcp.leases":["read"],"/proc/net/arp":["read"],"/proc/net/dev":["read"],"/bin/sh":["exec"]}},"write":{"ubus":{"*":["*"]},"uci":{"*":["read","write"]},"file":{"/bin/sh":["exec"],"/etc/init.d/firewall":["exec"],"/sbin/iptables":["exec"]}}}}\x1b[0m')
